...
export default function App() {
  ...
  const handleSafetyClearanceSubmit = async (clearedReq: BlockRequest) => {
    ...
    const updated = allRequests.map((r) => (r.id === clearedReq.id ? clearedReq : r));
    setAllRequests(updated);
    saveStoredRequests(updated);
    setActiveSafetyCheckoutRequest(null);
    if (activeDetailRequest?.id === clearedReq.id) {
      setActiveDetailRequest(clearedReq);
    }

    const engineerName = clearedReq.safetyClearedBy || currentUser?.name || 'Site Engineer';

    // 2. Real-time notification dispatch with audio chime to BOTH respective Department Officer AND Main Control Admin
    addNotification({
      type: 'STATUS_APPROVED',
    ...
    // 3. Update Supabase block_requests table
    try {
      const dbRes = await updateBlockRequestInSupabase(clearedReq);
      if (dbRes.success) {
        showToast(`Line Clear authorization for ${clearedReq.id} synced with Supabase cloud.`, 'success');
      }
    } catch (err) {
      console.warn('Supabase safety clearance update fallback:', err);
    }
  };
