...
def compatible(left: dict[str, Any], right: dict[str, Any]) -> bool:
    ...
    return max(0, max(left_start, right_start) - min(left_end, right_end)) <= ADJACENCY_MINUTES
...
def build_result(requests: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not pending:
        return {
            ...
            "percentHoursSaved": 0,
            "conflictReductionRatePercent": 100,
    ...
    conflict_rate = round(conflicts / compatible_pairs * 100) if compatible_pairs else 100
